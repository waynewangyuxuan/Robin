...
**Human 极度前置，AI 长时间自主 run，human 最后 verify。** Intake 是唯一 human 交互点；之后所有 stage 都 headless，按 budget 自 run 到完成或 degrade。

类比：P vs NP。Generate 难，verify 便宜。把所有需要 judgment 的事前置到 Intake，让后面几个小时的 execution 只需要 verify。
...
| Stage | 输入 | 输出 | 关键特征 |
|---|---|---|---|
| Intake | 用户原始 brief | intent / constraint / decision / convention / context 等 spec；完整 Feature Room 骨架 | 唯一和 human 交互的 stage；≤15 轮问答 budget |
| Planning | Intake 的 specs | 更多 decision spec、contract spec、带 gate 的 milestone 列表 | 最重要的产出是 contract spec（决定并行度） |
| Scheduler | plan + progress | `dispatch_batch` 信号（这批跑哪些 milestone、并发模式） | 纯 scheduler，无领域判断 |
...
Proxy decisions（Intake 替用户填的）特别标出来，在 intake_complete signal 的 `agent_proxy_decisions` 列表里，每条都有五段式 proxy note 说明 gap / 选择 / 理由 / hint / 什么会改变它。
...
| Kernel 的行为规范 | `skills/robin-kernel/discipline.md` |
| Spec 数据格式 | `stdlib/feature-room-spec.md` |
| 某个 stage 具体做什么 | `<stage>/SKILL.md` |
| Stage 内某一步的细节 | `<stage>/phases/phase-N-*.md` |
| Stage 的方法论 depth | `<stage>/<topic>.md`（例如 `skills/robin-planner/contract-design.md`） |
