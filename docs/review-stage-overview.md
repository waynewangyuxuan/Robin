...
Each playbook returns a sub-verdict
  ↓
Merger Agent consolidates into one merged verdict
  ↓
Kernel commits (hard rule), then routes next
```

This entry file (`docs/review-stage-overview.md`) is just a pointer. It tells main agent:
"spawn `skills/robin-review-planner/SKILL.md` to determine the review dispatch".
...
- `skills/robin-review-planner/SKILL.md` when main agent spawns it
- `skills/ (robin-reviewer-* dirs){name}/SKILL.md` for each playbook dispatch
- `skills/robin-merger/SKILL.md` when all playbooks have returned
...
2. **Main agent spawns Review-Plan**. Load `skills/robin-review-planner/SKILL.md`.
...
4. **Main agent spawns N review sub-agents in parallel**. For each
   ...
   - Load `skills/ (robin-reviewer-* dirs){playbook_name}/SKILL.md`
...
6. **Merger Agent returns `review_merged`**. Consolidates all sub-verdicts
   into overall pass/fail + consolidated issues.

7. **Kernel commits**. The merged verdict and all batch artifacts are
   committed to git (hard rule from skills/robin-kernel/discipline.md).

8. **Kernel routes**:
   - `overall_status: pass` or `pass_with_warnings` → Scheduler for
     next batch
...
The set of available playbooks lives in `skills/ (robin-reviewer-* dirs)`. Each is a
...
under `skills/ (robin-reviewer-* dirs){name}/SKILL.md`. Review-Plan Agent is responsible
...
Merger Agent's job is synthesis, not re-evaluation. If a playbook says
