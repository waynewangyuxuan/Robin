...
**源码是 runtime-agnostic 的**（抽象 NLP 的 SKILL / contracts / stdlib 等都
在 repo root），Claude Code plugin（`.claude-plugin/` + agents/ + commands/
+ hooks/）是第一个 runtime adapter。其他 runtime 可以增加 adapter 而不改源码。
...
```
/robin-start Build a Python CLI that prints fibonacci(10)
```

也可以：
- `/robin-resume` — 接着上次被打断的 run（会自动检测 `.ai-robin/stage-state.json`）
- `/robin-status` — 只看当前状态、不改 state

## 架构

- **抽象源**：`DESIGN.md` + `skills/robin-kernel/SKILL.md` + `skills/`
- **Claude Code adapter**：`.claude-plugin/` 下的 commands / agents / hooks
- **运行时状态**：每个项目下的 `.ai-robin/`（ledger、stage-state、dispatch inbox）

详见 [`DESIGN.md §8 Runtime adaptation`](DESIGN.md) 和
[`docs/plugin-equivalence.md`](docs/plugin-equivalence.md)
(plugin 保留什么、新加什么)。
...
的高带宽时间留给 Intake intake 和 final verify 两端，中间去做别的。
...
3. Robin plugin 已 install（`claude plugins install /path/to/AI-Robin-Skill`）
4. 一个干净的分支或者 worktree（Robin 会往里面频繁 commit）

### Run

在 Claude Code 里用 slash command 起一个 Robin run：

```
/robin-start I want to build [...]
```

然后 **Intake stage 会开始问你问题**——大约 4-8 个核心问题，最多 15 轮
对话。典型问题包括：
...
Intake 尽量给你多选项和默认值，不让你写长文。

Intake 完成后会告诉你：**"Intake complete. Starting autonomous run. 
...
2. **读 `META/00-project-room/spec.md`**：这是整个项目的 Human Projection，
   等于 Intake + Planning 产出的 intent / decision / contract 的渲染版
...
- Big miss → 重新 invoke AI-Robin，在 Intake stage 把原来的 miss 作为
...
如果 Intake 之后估算的 milestone 数超过 50（hard cap），Planning 会
...
在 `skills/` 下新建一个目录，写个 `SKILL.md`，描述触发条件
...
先看 **proxy decisions**（在 Intake 的 intake_complete signal 里，
也在 ledger 开头的 entries 里）。很多时候"结果不对"的根源是 Intake 替
你填的默认不符合你的真实偏好。

下次 run 的时候，在 Intake 那里主动把这些 proxy 点讲清楚。
...
Intake 完成后、在 Planning 开始前，你可以 kill 这次 run。Intake 的
产出（intent / decision / constraint / convention specs）已经落盘。
读 `META/00-robin-plan/specs/` 就知道 AI-Robin 理解到了什么。

不满意就重新 invoke 改 Intake 的回答。满意就让它继续。
...
- Main agent kernel
- Intake / Planning / Scheduler / Execute / Research agents
- Review-Plan + Merge
...
- `docs/architecture.md` —— 架构一页纸
- `docs/feature-room-mapping.md` —— 数据格式和 Feature Room 的兼容性
- `docs/plugin-equivalence.md` —— Claude Code plugin 和 source 的关系
