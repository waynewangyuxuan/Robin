...
The rules Execute-Control uses when deciding concurrency mode for a
batch. Used in Phase 3.

These rules operationalize the parallelism annotations Planning wrote
(per `agents/planning/parallelism-identification.md`). Where Planning
identified what CAN parallelize, Execute-Control decides what WILL
...
- Execute-Control's budget-aware batching (Phase 2) over-sized this
  batch
...
Shouldn't happen — Execute-Control Phase 2 picks at least one
...
platform cap; Execute-Control self-limits via batch-size rules in
Phase 2.
