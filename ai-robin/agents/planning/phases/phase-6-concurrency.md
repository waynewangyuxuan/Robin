...
Load `agents/planning/parallelism-identification.md` for full methodology.
...
- Optionally **`parallel_safe_with`** listing sibling milestones this
  one is explicitly safe to run alongside (Execute-Control uses this
  as a hint)
...
## Output feeds Execute-Control

Execute-Control reads these annotations when forming batches. Good
concurrency annotations let Execute-Control dispatch 3-5 parallel
