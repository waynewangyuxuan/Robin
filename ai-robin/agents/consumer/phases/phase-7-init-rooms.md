# Consumer Phase 7: Initialize Room structure
...
│   ├── progress.yaml
│   └── specs/
└── 00-ai-robin-plan/         # Empty, for Planning Agent
    ├── room.yaml
    └── specs/
```
...
Consumer does **epic-level** decomposition only. Planning does finer
decomposition.

- Single-scope or tiny project: 1 feature room + `00-project-room` +
  `00-ai-robin-plan` = 3 rooms total
- Medium project: 2-5 feature rooms + infrastructure
- Large project: 3-8 feature rooms

**Never more than 8 rooms at Consumer stage.** If you're tempted, the
extras are either:
...
Consumer does not write anything into this room.
...
- Do not skip creating `00-ai-robin-plan` — Planning will fail without
  it.
