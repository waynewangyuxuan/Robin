...
- **Decision specs** → `META/00-ai-robin-plan/specs/decision-*.yaml`
...
- **Milestones** → each affected Room's `progress.yaml`, plus the
  master list in `META/00-ai-robin-plan/progress.yaml`

## State

All specs `state: active`. Planning is post-Consumer; no human review
intercepts its output. Downstream agents (Execute-Control, Execute)
...
If a spec already exists (e.g., Consumer's or from a previous Planning
