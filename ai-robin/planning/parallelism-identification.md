...
by Execute-Control until A is `completed`.
...
Execute-Control uses this to avoid placing both into the same batch.

### `parallel_safe_with` (optional hint)

For milestones where parallel safety is non-obvious, annotate
explicitly. Helps Execute-Control decide confidently.
...
intent:
  ...
  detail: |
    ...
    Enforcement: Execute-Control respects this when batching; Review
    flags if violations occur.
```

These architectural specs are referenced by any milestone touching the
affected files, so Execute-Control sees the constraint when planning
batches.
...
**Annotations**: none needed; Execute-Control can dispatch all three
...
Execute-Control can parallelize m2 with *siblings* of m2, but not with
m1 itself.
...
## What Execute-Control does with these annotations

Execute-Control reads the plan in its Phase 2-3:
...
Execute-Control uses these to form valid batches.
