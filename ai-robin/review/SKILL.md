...
This entry file (`review/SKILL.md`) is just a pointer. It tells main agent:
"spawn `review/review-plan/SKILL.md` to determine the review dispatch".
...
- `review/review-plan/SKILL.md` when main agent spawns it
- `review/playbooks/{name}/SKILL.md` for each playbook dispatch
- `review/merge/SKILL.md` when all playbooks have returned
...
2. **Main agent spawns Review-Plan**. Load `review/review-plan/SKILL.md`.
...
4. **Main agent spawns N review sub-agents in parallel**. For each
   ...
   - Load `review/playbooks/{playbook_name}/SKILL.md`
...
7. **Kernel commits**. The merged verdict and all batch artifacts are
   committed to git (hard rule from kernel-discipline.md).
...
The set of available playbooks lives in `review/playbooks/`. Each is a
...
under `review/playbooks/{name}/SKILL.md`. Review-Plan Agent is responsible
