...
**Plan file location note:** This document lives at `references/plan-2-plugin-migration.md` at plan-start. During **Phase 1 Task 1.3**, `references/` is renamed to `docs/`, so this file's final location will be `docs/plan-2-plugin-migration.md`. Executors must not treat the rename as breaking this plan.
...
│   ├── kernel/
│   │   └── discipline.md                 # was stdlib/kernel-discipline.md
│   ├── consumer/                         # moved from ai-robin/consumer/
│   │   ├── SKILL.md
...
- Move: `ai-robin/stdlib/kernel-discipline.md` → `ai-robin/agents/kernel/discipline.md`
...
```bash
git mv stdlib/kernel-discipline.md agents/kernel/discipline.md
```
...
### Task 1.4: Rename references/ → docs/

**Files:**
- Move: `ai-robin/references/` → `ai-robin/docs/`

**Important:** this plan file itself is currently at `ai-robin/references/plan-2-plugin-migration.md`. After this task, it will be at `ai-robin/docs/plan-2-plugin-migration.md`. Downstream tasks refer to the new location.
...
| `execute-control/` | `agents/execute-control/` |
| `execute/` | `agents/execute/` |
| `research/` | `agents/research/` |
| `review/` | `agents/review/` |
| `stdlib/kernel-discipline.md` | `agents/kernel/discipline.md` |
| `references/` | `docs/` |
...
s|(^|[^a-zA-Z-])consumer/|\1agents/consumer/|g
s|(^|[^a-zA-Z-])planning/|\1agents/planning/|g
s|(^|[^a-zA-Z-])execute-control/|\1agents/execute-control/|g
s|(^|[^a-zA-Z/-])execute/|\1agents/execute/|g
s|(^|[^a-zA-Z-])research/|\1agents/research/|g
s|(^|[^a-zA-Z-])review/|\1agents/review/|g
' {} \;
...
# references/ → docs/
find . -type f -name '*.md' -exec sed -i '' \
  -e 's|references/|docs/|g' {} \;
```

**Note on sed flavor:** the `-i ''` syntax is for BSD sed (macOS). On GNU sed, use `-i` (no empty string).

**Note on `execute/` replacement:** the negative char class `[^a-zA-Z/-]` prevents matching `agents/execute/` (because `/` precedes). If run twice, the regex self-protects.
...
```bash
cd /Users/waynewang/AI-Robin-Skill/ai-robin
echo "=== Bare 'consumer/' not under agents/ ==="
grep -rn --include='*.md' -E '[^a-zA-Z/-]consumer/' . | grep -v 'agents/consumer/' | head -20

echo "=== Bare 'planning/' not under agents/ ==="
grep -rn --include='*.md' -E '[^a-zA-Z/-]planning/' . | grep -v 'agents/planning/' | head -20

echo "=== kernel-discipline.md in stdlib/ ==="
grep -rn --include='*.md' 'stdlib/kernel-discipline' . | head -20

echo "=== references/ still around ==="
grep -rn --include='*.md' 'references/' . | head -20
```
...
```bash
grep -E 'consumer/|planning/|stdlib/kernel-discipline' DESIGN.md
```
...
```bash
grep -n 'consumer/\|planning/\|execute/\|execute-control/\|research/\|review/\|stdlib/' tests/end-to-end-trace.md
```
...
```bash
grep -E 'consumer/|planning/|stdlib/kernel-discipline' SUMMARY.md
```
...
```bash
cd /Users/waynewang/AI-Robin-Skill
grep -n 'consumer/\|planning/\|execute/\|stdlib/kernel-discipline\|references/' README.md
```
...
Moves:
- consumer/ planning/ execute-control/ execute/ research/ review/ → agents/
- stdlib/kernel-discipline.md → agents/kernel/discipline.md
- references/ → docs/
