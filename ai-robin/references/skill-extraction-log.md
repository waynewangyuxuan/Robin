...
| `execute/context-pulling.md` | `prompt-gen` skill | context 加载规则；state-aware filtering；relations 遍历；什么该 / 不该加载 |
| `execute/commit-preparation.md` | `commit-sync` skill Phase 1-4 | change spec 格式；progress.yaml 更新；commit message 模板 |
...
| AI-Robin 文件 | 抽取自 | 状态 |
|---|---|---|
| `review/playbooks/code-quality/SKILL.md` | 2025 业界 code review checklist（Google、Microsoft、OWASP 的公开 practice，综合多篇 2025 年 blog 文章） | ✅ 已写 |
...
| `stdlib/kernel-discipline.md` | Main agent 永远 light 的规则 |
...
| `consumer/*.md` | Consumer 是 AI-Robin 特有的 stage |
| `planning/*.md` | Planning 是 AI-Robin 特有的 stage（Feature Room 没有单独的 Planning agent） |
| `execute-control/*.md` | 调度层是 AI-Robin 特有 |
| `review/*.md` | Plan-fan-out-merge review 结构是 AI-Robin 特有 |
| `research/SKILL.md` | Research agent 是 AI-Robin 特有；简版，用户自定义 |

### 部分借鉴（不是直接抽取）

- Consumer 的 gap analysis + 替决策填默认的方法（`consumer/phase-2-gap-analysis.md`、`consumer/phase-6-proxy.md`）借鉴了 Feature Room 里 Q&A 穷举 gap 的思路，但具体的五段式 proxy note 和 15 轮交互 budget 是新的。
- Planning 的 contract design 方法论（`planning/contract-design.md`）参考了一般软件工程里"设计 API 契约"的 best practice（exposed / invariants / preconditions 三段式），但作为 AI-Robin 特有的 depth file 写成。
- Execute-Control 的并发规则（`execute-control/concurrency-rules.md`）是在 Feature Room 的 cross-room-conflict-detection 思路基础上扩展了"single-writer rule"、"schema/migration implicit serial"等 AI-Robin 特有的规则。
...
→ 重新 review `execute/commit-preparation.md` 和 `stdlib/anchor-tracking.md`。
...
→ 更新对应的 `review/playbooks/*/SKILL.md`。
