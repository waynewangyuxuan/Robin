...
The last gate before Intake returns. Used in Phase 9 (Self-Check).

Intake is the entry point for the entire AI-Robin run. A premature
`intake_complete` with gaps is the single worst Intake failure mode —
it causes Planning to consume replan budget on gaps Intake should
...
`skills/robin-intake/decision-taxonomy.md`'s must-ask list for that type. For each
item, verify:
...
- Each room has the four files: `room.yaml`, `spec.md`, `progress.yaml`,
  `specs/`
- `00-robin-plan/` exists and is empty (Planning's workspace)
- `00-project-room/_tree.yaml` indexes all rooms

Typical target: 1-8 feature rooms + `00-project-room` + `00-robin-plan`.
...
through what Planning does (per `skills/robin-planner/SKILL.md`):
...
Intake, that's a failure of Check 6.
...
- **The `00-robin-plan/` room exists.** Planning will fail without
  it.
