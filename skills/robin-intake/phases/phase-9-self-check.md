# Intake Phase 9: Completeness self-check
...
Load `skills/robin-intake/completeness-check.md` for the detailed checklist. This
phase applies it.
...
Intake's output. Walk through what Planning does (per
`skills/robin-planner/SKILL.md`):
...
Intake, that's a failure of this check.
...
A Intake that returns `intake_complete` prematurely with gaps is
