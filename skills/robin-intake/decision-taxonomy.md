...
The catalog of decision points per project type. Intake Agent uses this
...
(Intake Phase 1), find the matching section, walk the decision points,
classify each per Phase 2's four buckets (covered / derivable / proxy-able /
must-ask).

The defaults listed are Intake's go-to choices when proxying (Phase 6).
...
As AI-Robin runs on new project types, Intake will hit gaps in this
catalog. When that happens:

1. The run's Intake can still proceed by asking the user directly for
...
This file is meant to be extended over time. Each addition makes Intake
