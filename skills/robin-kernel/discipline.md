...
```
sub_agent: planning
skill_path: skills/robin-planner/SKILL.md
task_spec: {
...
- This is the first turn and we need to kick off Intake Agent
...
handles the "fresh run, spawn Intake" initialization). Even if multiple
...
supported by AI-Robin's design** (only one human interaction, at Intake).

Respond:

> "AI-Robin's current design only takes input at intake (Intake Agent).
...
|---|---|
| "Let me check if this planning output looks reasonable before spawning Scheduler" | No. Signal arrived with `planning_complete`. Route per the table. The signal's producer (Planning Agent) is responsible for its quality, not you. |
...
|---|---|---|
| `review_merged` arrives (pass or fail) | Commit Agent | Running `git add` / `git commit` with the verdict message requires the domain-aware `commit_message` from Merger Agent; kernel passes it through verbatim but never reads its content or synthesizes its own. |
