...
之后每个 session 都有 `/robin-start`、`/robin-resume`、`/robin-status`。Claude Code 每次启动会自动从本仓库 default branch 拉最新版。

### 本地开发安装

如果你在改 Robin 源码、希望改动实时生效（不 copy、不 pin commit）：

```bash
git clone https://github.com/waynewangyuxuan/Robin.git
cd Robin
source ./dev-install.sh             # 写入 `claude-robin` alias，当前 shell 立即可用
claude-robin                        # 起 session，Robin 从你的源码目录加载
```

Mid-session 改完 skills / agents / hooks 跑 `/reload-plugins` 热更新，无需重启。卸载 alias：`./dev-install.sh remove`。
